...
with st.sidebar:
    ...
    st.subheader('Create Random Data Set')
    n_samples = st.number_input('Enter No.of Samples (n):', min_value=0, max_value=100000, value="min", placeholder=1000)
    random_state = st.number_input('Enter Randomstate:', min_value=0, max_value=100000, value="min", placeholder='Enter any integer...')

    from sklearn.datasets import make_classification
    fv, cv = make_classification(n_samples=int(n_samples), n_features=2, n_informative=2, n_redundant=0, n_repeated=0, 
                                 n_classes=2, n_clusters_per_class=1, class_sep=2.0, random_state=random_state)
